...
## layour
layout(mat = matrix(c(rep(1, 3), seq(2, l = 9)), ncol = 4),
...
corrplot.mixed(R,
               ...
               lower.col = 1, mar = c(0, 0, 1, 0))
lapply(1:ncol(da), function(i) {
    par(mar = c(2.5, 1, 2.5, 1) + 0.1)
    d <- density(da[, i])
    plot(d$x, d$y, type = "n",
         xlab = "", ylab = "",
         main = parse(
             text = gsub(":", replace = "", colnames(R)[i])
         ), axes = FALSE)
    axis(1)
    grid()
    box()
...
model <- principal(da, nfactors = 3, rotate = 'varimax')
...
```{r, fig.height=5, fig.width=10, fig.cap="Avaliação da qualidade de ajuste do modelo. Distribuição dos resíduos calculados pela diferença entre $R$ e $LL^t + \\Psi$ (esquerda superior) e representação da matriz de resíduos (esquerda superior) e escores dos países com base nos três fatores obtidos (direita)."}
...
xy3 <- splom(~scores,
             ...
             pscales = apply(scores, 2, function(x) {
                 list(limits = extendrange(x, f = 0.1))
             }),
             diag.panel = function(x, ...){
                 yrng <- current.panel.limits()$ylim
                 d <- density(x, na.rm = TRUE)
             ...
             panel = function(x, y, groups, ...) {
                 ...
                 panel.points(x, y, col = "gray50", pch = 19,
                              cex = 1.1, alpha = 0.8)
                 panel.text(x, y - 0.1, groups, cex = 0.65)
             })
