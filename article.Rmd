...
textos em uma matrix $\mathbb{X}$, cujo linhas são os documentos e
...
associadas ao termo `cluster`. Existem vários algoritmos para
...
interesse em análise de texto a redução do número de colunas da matrix
...
Seja $\mathbf{R}$ a matriz de correlações de $\mathbb{X}$, de dimensão
$p\times p$. $\mathbf{R}$ pode ser escrita como $\mathbf{R} = \mathbf{E}
\Lambda \mathbf{E}^t$, em que $\Lambda$ é a matriz diagonal dos
autovalores $\lambda_i$, $i=1,\ldots ,p$ da matriz $\mathbb{X}$, ou
...
Note que são tomadas calculadas tantas componentes quanto for o número
de variáveis, porém apenas um subconjunto dessas componentes já é
...
agrupamentos hieráquicos e não-hierárquicos. Nos hierárquicos
...
procedimento para formação dos grupos via _k-means_. Note que os dados
de entrada do algortimo é o número de grupos a serem formados e os
dados.
...
\begin{algorithm}[H]
  ...
  \While{As atribuições dos grupos não for a mesma.}{\label{line:while}
    \textbf{(a)} Para cada grupo $k$ calcule o seu centróide. O
    centróide do $k$-ésimo grupo é dado pelo vetor média das $p$
    variáveis calculado das observação pertencentes a esse grupo;\\
    \textbf{(b)} Atribua cada observação ao grupo, cujo tem o centróide
     mais próximo, a proximidade é dada distância euclidiana entre o
     centróide e o vetor de variáveis da observação
  }
\end{algorithm}
\vspace{0.5cm}

Conforme visto no \autoref{alg:kmeans} o número de grupos deve ser
informado a priori, para avaliação do agrupamento realizado pode-se
verificar as medidas de homogeneidade dos grupos e heterogeneidade entre
grupos. A homogeneidade é dada pelo inverso da soma de quadrados das
observações alocadas em um mesmo grupos e a heterogeneidade pela soma de
quadrado das observações alocadas em grupos distintos.

O principal problema em cados de agrupamento é a escolha do número de
grupos, $k$, a serem formados. @Tibshirani2001 propôs a estatística Gap
para esttimação de $k$. Essa estatística é calculada usando as medidas
de homogeneidade do agrupamento em contraste com medidas de variáveis
simuladas, em geral, por uma distribuição uniforme. A escolha do número
de grupos é então realizada comparando as estatística Gap para um $k$.
...
disponíveis no sitío eletrônico do autor
...
extraídos. Após pré-processamento os `r nar` artigos forneceram `r nx`
...
que o argrupamento conseguiu juntar artigos com conteúdos similares
...
modelos de probabísticos de tópicos que podem auxiliar na formação dos
